...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'All Sites':
        fig = px.pie(spacex_df, values='class', names='Launch Site', title='Total Success by Launch Site')
        return fig
    else:
        # return the outcomes piechart for a selected site
        filtered_df =  spacex_df[spacex_df['Launch Site'] == entered_site]
        filtered_df['Count'] =  1
        fig=px.pie(filtered_df, values='Count', names='class', title='Total Success for Launch site '+entered_site)
        return fig
...
def get_pie_chart(entered_site, selected_payload):
    filtered_df = spacex_df
    if entered_site == 'All Sites':
